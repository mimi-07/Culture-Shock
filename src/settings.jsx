...
function Settings() {
  ...
  const customerId = paramCustomerId || localStorage.getItem("shopifyCustomerId");
  const [customer, setCustomer] = useState(null);
  const storedCustomerId = localStorage.getItem("shopifyCustomerId");
  if (!storedCustomerId || storedCustomerId !== customerId) {
    // ✅ If not logged in OR if the URL customerId doesn't match the stored ID, redirect to login
  ...
  const handleLogoutEverywhere = async () => {
    try {
        const response = await fetch("http://localhost:5002/logout-everywhere", {
            method: "POST",
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      const idToUse = customerId || storedCustomerId;

      if (!idToUse) return;

      try {
        const response = await fetch(`http://localhost:5002/customer/${idToUse}`);
        const data = await response.json();
      ...
      <div className='container'>
        <div className="navbar_1">
          <a href="#/" className="logo">
            <ul>
              <li><img src="/Culture-Shock/images/logo.png" alt="Logo" /></li>
            </ul>
          </a>
          <nav class="icons">
        ...
        <section id="contact" style={{display:"flex",flexDirection:"column",gap:"10px "}}>
           <h1>Settings </h1>
           <h2 className='LGE1'><i class='bx bx-lock-alt'></i>Log out everywhere</h2>
           <p style={{color:"rgb(166, 88, 88)"}}>If you've lost a device or have security concerns, log out everywhere to ensure the security of your account.</p>
           <div className='LGE2'>
            <button onClick={handleLogout}>Log out Everywhere</button>
